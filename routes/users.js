...
app.post("/setUserType",function(req,res){
    var user
    User.find({_id:req.user._id},function(err,data){
        if(err) throw err
        user = data[0]

        //req.body is true or false
         if(user.moreInfo === undefined){
            user.moreInfo = {}
        }
        user.moreInfo.userType =  req.body.userType
...
app.post("/newFullName",function(req,res){
    ...
    User.find({_id:req.user._id},function(err,data){
        ...
        user.moreInfo.fullName = req.body.fullName
